...
import { offlineStorage, OfflineOrder } from './offline-storage';
...
class SyncManager {
  ...
  private async syncOrder(order: OfflineOrder): Promise<void> {
    try {
      await offlineStorage.updateOrderStatus(order.id, 'syncing', order.syncAttempts + 1);
      ...
      const maxAttempts = 3;
      if (order.syncAttempts >= maxAttempts) {
        await offlineStorage.updateOrderStatus(order.id, 'error', order.syncAttempts);
      } else {
        await offlineStorage.updateOrderStatus(order.id, 'pending_sync', order.syncAttempts);
      }
    }
  }
